...
with tab2.container():
        st.title("   Order Details Analysis  ")
        inputs = st.sidebar.selectbox('Queries', [
        ...
        detail = {'The top 5 sub catogery with highest quantity of orders' : """SELECT o.sub_category, SUM(p.quantity) AS Quantity
        ...
        ax.set_xlabel(column_names[0])
        ax.set_ylabel(column_names[1])
        ax.set_title(f"Chart based on Query: {inputs}")
        st.pyplot(fig)
